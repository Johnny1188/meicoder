...
class L2O_Decoder(nn.Module):
    def __init__(
        ...
        resp_layers_batch_norm=True,
        resp_layers_dropout=0.0,
        ...
        self.bottleneck_in_shape = (16, 4, 4)
        self.bottleneck_out_shape = (16, 6, 6)
    ...
    # @timeit
    def _get_resp_err_grad(self, x_hat, resp_target):
        resp_x_hat = self.encoder(x_hat)
    ...
    # @timeit
    def _init_reconstruction(self, resp, track_grad=True):
    ...
    # @timeit
    def forward(self, x_hat, x_hat_grad, x_hat_t0, hidden, cell):
        ### construct input
    ...
    def run_batch(self, stim, resp, n_steps=1, train=False, x_hat_history_iters=None):
        ...
        B = resp.shape[0]
        x_hat = self._init_reconstruction(resp=resp, track_grad=True) # (B, C, H, W)
        ...
        for step_i in range(1, n_steps + 1):
            ...
            x_hat_grad = self._get_resp_err_grad(x_hat=x_hat, resp_target=resp).detach()
            if self.preproc_grad:
                x_hat_grad = self._preproc_grad(x_hat_grad)

            ### update the decoded image
            x_hat_delta, hidden, cell = self.forward(
                x_hat=x_hat,
