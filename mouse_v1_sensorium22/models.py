    ...
    def set_additional_loss(self, inp, out):
        for readin_data_key, readin in self.readins.items():
            if "data_key" in inp and inp["data_key"] != readin_data_key:
                continue
            for l in readin:
                out["encoded"] = l(inp["resp"], neuron_coords=inp["neuron_coords"], pupil_center=inp["pupil_center"])
...
class ConvReadIn(ReadIn):
    ...
    def set_additional_loss(self, inp, out):
        ...
        elif hasattr(self, "grid_l1_reg") and self.grid_l1_reg > 0:
    ...
    def forward(self, x, neuron_coords, pupil_center):
        ...
        if self.learn_grid:
            ...
            ], dim=-1) # (B, n_neurons, 3 + 1)
            grid_net_inp = grid_net_inp.view(B * n_neurons, 4)
...
#         ### cnn decoder ("core")
#         self.gan_config = deepcopy(gan_config)
#         self.gan = GAN(**self.gan_config)
...
#                     readin.append(nn.Linear(in_channels, out_channels))
#                 elif layer_config[0] == "unflatten":
#                     layer_type, in_dim, unflattened_size = layer_config
#                     readin.append(nn.Unflatten(in_dim, unflattened_size))
#                     out_channels = unflattened_size[0]
#                 elif layer_config[0] == "hypernet_readin":
#                     layer_type, hypernet_readin_config = layer_config
#                     readin.append(HypernetReadIn(**hypernet_readin_config))
#                     out_channels = hypernet_readin_config["target_layers"][-1][-1][0]
#                 elif layer_config[0] == "conv_readin":
#                     layer_type, conv_readin_config, out_dim = layer_config
#                     readin.append(ConvReadIn(**conv_readin_config))
...
#                     if self.gan_config["batch_norm"]:
#                         readin.append(nn.BatchNorm1d(out_channels))
#                     readin.append(self.gan_config["act_fn"]())
#                     if self.gan_config["dropout"] > 0.0:
#                         readin.append(nn.Dropout(self.gan_config["dropout"]))

#                 in_channels = out_channels

#             if not hasattr(readin[0], "requires_neuron_coords") \
#              or not readin[0].requires_neuron_coords:
#                 self.readins[readin_config["data_key"]] = nn.Sequential(*readin)
#             else:
#                 self.readins[readin_config["data_key"]] = nn.ModuleList(readin)
...
#     @property
#     def D_optim(self):
#         return self.gan.D_optim

#     @property
#     def G_optim(self):
#         return self.gan.G_optim

#     def forward(self, x, data_key, neuron_coords=None, pupil_center=None):
#         ### run readin layer
#         if type(self.readins[data_key]) == nn.Sequential:
#             x = self.readins[data_key](x)
#         else:
#             for l in self.readins[data_key]:
#                 x = l(x, neuron_coords=neuron_coords, pupil_center=pupil_center)
...
# class MultiReadInCNN(nn.Module):
#     def __init__(self, readins_config, cnn_config, crop_stim_fn=None):
#         super().__init__()

#         ### cnn decoder ("core")
#         self.cnn_config = deepcopy(cnn_config)
#         self.cnn = CNN_Decoder(**self.cnn_config)
...
#                     readin.append(ConvReadIn(**conv_readin_config))
#                     out_channels = out_dim
#                 elif layer_config[0] == "attn_readin":
#                     layer_type, attn_readin_config = layer_config
#                     readin.append(AttentionReadIn(**attn_readin_config))
#                     out_channels = readin[-1].out_channels
#                 else:
#                     raise ValueError(f"layer_type {layer_type} not recognized")

#                 if layer_type in ["fc"]:
#                     ### add batch norm, activation, dropout
#                     if self.cnn_config["batch_norm"]:
#                         readin.append(nn.BatchNorm1d(out_channels))
#                     readin.append(self.cnn_config["act_fn"]())
#                     if self.cnn_config["dropout"] > 0.0:
#                         readin.append(nn.Dropout(self.cnn_config["dropout"]))
