...
from csng.utils import seed_all, plot_losses, plot_comparison, standardize, normalize, get_mean_and_std, count_parameters, crop
...
if "brainreader_mouse" in cfg["data"]:
    ...
    for data_key, dset in _dls["train"]["brainreader_mouse"].datasets.items():
        ...
        cfg["decoder"]["loss"]["loss_fn"][data_key] = SSIMLoss(window=cfg["crop_wins"][data_key], log_loss=True, inp_normalized=True, inp_standardized=False)
        cfg["decoder"]["loss"]["con_reg_loss_fn"][data_key] = SSIMLoss(window=cfg["crop_wins"][data_key], log_loss=True, inp_normalized=True, inp_standardized=False)

        ### append readin
        n_neurons = dset[0].responses.shape[-1]
        cfg["decoder"]["model"]["readins_config"].append({
            "data_key": data_key,
            "in_shape": n_neurons,
...
if "cat_v1" in cfg["data"]:
    ### set crop wins and losses
    cfg["crop_wins"]["cat_v1"] = cfg["data"]["cat_v1"]["crop_win"]
    cfg["decoder"]["loss"]["loss_fn"]["cat_v1"] = SSIMLoss(window=cfg["data"]["cat_v1"]["crop_win"], log_loss=True, inp_normalized=True, inp_standardized=False)
    cfg["decoder"]["loss"]["con_reg_loss_fn"]["cat_v1"] = SSIMLoss(window=cfg["data"]["cat_v1"]["crop_win"], log_loss=True, inp_normalized=True, inp_standardized=False)
...
if "mouse_v1" in cfg["data"]:
    _dls, _neuron_coords = get_dataloaders(config=cfg)
    for data_key, n_coords in _dls["train"]["mouse_v1"].neuron_coords.items():
        ### set crop wins and losses
        cfg["crop_wins"][data_key] = cfg["data"]["mouse_v1"]["crop_win"]
        cfg["decoder"]["loss"]["loss_fn"][data_key] = SSIMLoss(window=cfg["data"]["mouse_v1"]["crop_win"], log_loss=True, inp_normalized=True, inp_standardized=False)
        cfg["decoder"]["loss"]["con_reg_loss_fn"][data_key] = SSIMLoss(window=cfg["data"]["mouse_v1"]["crop_win"], log_loss=True, inp_normalized=True, inp_standardized=False)
