...
from csng.gan_decoder_utils import init_decoder, setup_run_dir, setup_wandb_run, train, val
...
if "brainreader_mouse" in cfg["data"]:
    ...
    for data_key, dset in _dls["train"]["brainreader_mouse"].datasets.items():
        ...
        cfg["decoder"]["model"]["core_config"]["D_kwargs"]["layers"][-1][data_key] = {
            "in_shape": [1, 36, 64],
            "layers_config": [("fc", 1)],
            "act_fn": nn.Identity,
...
if "cat_v1" in cfg["data"]:
    ...
    cfg["decoder"]["loss"]["loss_fn"]["cat_v1"] = SSIMLoss(window=cfg["crop_wins"]["cat_v1"], log_loss=True, inp_normalized=True, inp_standardized=False)
    cfg["decoder"]["loss"]["con_reg_loss_fn"]["cat_v1"] = SSIMLoss(window=cfg["crop_wins"]["cat_v1"], log_loss=True, inp_normalized=True, inp_standardized=False)

    ### append discriminator's head
    cfg["decoder"]["model"]["core_config"]["D_kwargs"]["layers"][-1]["cat_v1"] = {
        "in_shape": [1, *cfg["crop_wins"]["cat_v1"]],
        "layers_config": [("fc", 1)],
        "act_fn": nn.Identity,
        "out_act_fn": nn.Sigmoid,
    }
...
if "mouse_v1" in cfg["data"]:
    ...
    for data_key, n_coords in _dls["train"]["mouse_v1"].neuron_coords.items():
        ...
        ### append discriminator's head
        cfg["decoder"]["model"]["core_config"]["D_kwargs"]["layers"][-1][data_key] = {
            "in_shape": [1, *cfg["crop_wins"][data_key]],
            "layers_config": [("fc", 1)],
            "act_fn": nn.Identity,
            "out_act_fn": nn.Sigmoid,
        }
