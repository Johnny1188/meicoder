...
from csng.brainreader_mouse.gan_utils import train, val
...
if "brainreader_mouse" in config["data"]:
    ...
    for data_key, dset in _dls["train"]["brainreader_mouse"].datasets.items():
        config["crop_wins"][data_key] = config["data"]["brainreader_mouse"]["crop_win"]
        ...
        config["decoder"]["loss"]["con_reg_loss_fn"][data_key] = SSIMLoss(window=config["crop_wins"][data_key], log_loss=True, inp_normalized=True, inp_standardized=False)
...
if __name__ == "__main__":
    ...
    ### sample data
    stim, resp, sample_data_key, sample_dataset = None, None, None, None
    if "brainreader_mouse" in config["data"]:
        b_sample_dataset = "brainreader_mouse"
        b_dp = next(iter(dls["val"][b_sample_dataset]))
        b_stim, b_resp, b_sample_data_key = b_dp[0]["stim"], b_dp[0]["resp"], b_dp[0]["data_key"]
        stim, resp, sample_data_key, sample_dataset = b_stim, b_resp, b_sample_data_key, b_sample_dataset
    if "cat_v1" in config["data"]:
        c_sample_dataset = "cat_v1"
        c_dp = next(iter(dls["val"][c_sample_dataset]))
        c_stim, c_resp, c_sample_data_key = c_dp[0]["stim"], c_dp[0]["resp"], c_dp[0]["data_key"]
        stim, resp, sample_data_key, sample_dataset = c_stim, c_resp, c_sample_data_key, c_sample_dataset
    if "mouse_v1" in config["data"]:
        ...
        stim, resp, sample_data_key, sample_dataset = m_stim, m_resp, m_sample_data_key, m_sample_dataset
    ...
    with torch.no_grad():
        stim_pred = decoder(resp.to(config["device"]), data_key=sample_data_key, neuron_coords=neuron_coords[sample_dataset][sample_data_key])
    ...
    for epoch in range(s, e):
        ...
        if "brainreader_mouse" in config["data"]:
            b_stim_pred = decoder(b_resp[:8].to(config["device"]), neuron_coords=neuron_coords[b_sample_dataset][b_sample_data_key], data_key=b_sample_data_key).detach()
            fig = plot_comparison(target=b_stim[:8].cpu(), pred=b_stim_pred[:8].cpu(), save_to=make_sample_path(epoch, "b_"), show=False)
        if "cat_v1" in config["data"]:
            c_stim_pred = decoder(c_resp[:8].to(config["device"]), neuron_coords=neuron_coords[sample_dataset][c_sample_data_key], data_key=sample_data_key).detach()
            fig = plot_comparison(target=c_stim[:8].cpu(), pred=c_stim_pred[:8].cpu(), save_to=make_sample_path(epoch, "c_"), show=False)
        if "mouse_v1" in config["data"]:
    ...
    if "brainreader_mouse" in config["data"]:
        b_stim_pred_best = decoder(b_resp[:8].to(config["device"]), neuron_coords=neuron_coords[b_sample_dataset][b_sample_data_key], data_key=b_sample_data_key).detach().cpu()
        fig = plot_comparison(
            target=b_stim[:8].cpu(),
            pred=b_stim_pred_best[:8].cpu(),
            show=False,
            save_to=os.path.join(config["dir"], "b_stim_comparison_best.png") if config["save_run"] else None,
        )
    if "cat_v1" in config["data"]:
        c_stim_pred_best = decoder(c_resp[:8].to(config["device"]), neuron_coords=neuron_coords[sample_dataset], data_key=sample_data_key).detach().cpu()
        fig = plot_comparison(
            target=c_stim[:8].cpu(),
            pred=c_stim_pred_best[:8].cpu(),
            show=False,
