...
def main():
    ...
    # Test also activation = "sigmoid"
    model = get_model("deconv", activation="relu").to(DEVICE)
    # Test different values for lr (1e-1, 1e-2, 1e-3) and weight_decay (0.1, 1, 10)
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=3e-4, weight_decay=0.03
    )
    ...
    torch.save(model.state_dict(), f"deconv.pth")
    print(f"Saved model.")
